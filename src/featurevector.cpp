...
{
    int *mat = new int[mData.size() * FeatureData::HistogramLength];
    int row = 0;
    for (auto it = mData.begin(); it != mData.end(); ++it, ++row)
    {
        const int *ptr = nullptr;
        if (it->second.GetHistogram(&ptr))
        {
            memcpy(mat + row * FeatureData::HistogramLength, ptr, FeatureData::HistogramLength * sizeof(int));
        }
        else
        {
            assert(0);
    ...
    SVD(mat, mData.size(), FeatureData::HistogramLength);
}

bool Alisa::ImageFeatureVector::IsoData()
{
    ...
    for (auto it = mData.begin(); it != mData.end(); ++it)
    {
        ...
        if (it->second.GetHistogram(&ptr))
        {
            input_data[idx].Set(ptr, it->second.GetFilename().c_str());
            assert(input_data[idx].totle_count == it->second.GetPixelCount());
        }
        else
        {
            assert(0);
